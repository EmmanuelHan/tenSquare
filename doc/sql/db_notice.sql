...
CREATE TABLE `tb_notice` (
  ...
  `receiver_id` varchar(20) DEFAULT NULL COMMENT '接收消息用户的ID',
  `operator_id` varchar(20) DEFAULT NULL COMMENT '进行操作用户的ID',
  `action` varchar(255) DEFAULT NULL COMMENT '操作类型（评论，点赞等）',
  `target_type` varchar(255) DEFAULT NULL COMMENT '被操作的对象，例如文章，评论等',
  `target_id` varchar(20) DEFAULT NULL COMMENT '被操作对象的id，例如文章的id，评论的id',
...
CREATE TABLE `tb_notice_fresh` (
  ...
  `notice_id` varchar(20) DEFAULT NULL COMMENT '通知id'
